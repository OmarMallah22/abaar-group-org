...
const AboutClient: React.FC<AboutClientProps> = ({ initialTeam, initialStats }) => {
    ...
    return (
        <div className="bg-white min-h-screen font-arabic overflow-x-hidden" dir="rtl">
            ...
            {/* --- Leadership Team Section (حل نهائي لمشكلة التداخل وانضغاط الكروت) --- */}
            <section ref={teamSectionRef} className="py-16 md:py-28 bg-slate-50 overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-16">
                        <h2 className="text-3xl md:text-5xl font-black text-slate-800 mb-6">فريق أبار جروب</h2>
                            ...
                            <div className="px-2 md:px-0">
                                <Slider {...leadershipSettings}>
                                    {teamMembers.map((member) => (
                                        <div key={member.id} className="px-2 md:px-4 pb-10">
                                            <div className="bg-white rounded-[2.5rem] shadow-xl p-8 md:p-12 text-center border border-slate-100 transition-all duration-500 hover:-translate-y-2 flex flex-col items-center justify-center min-h-[380px] group">
                                                <div className="w-24 h-24 md:w-32 md:h-32 bg-sky-50 rounded-full flex items-center justify-center mb-6 overflow-hidden border-4 border-white shadow-md relative">
                                                    {member.image_url ? (
                                                        <Image 
                                                ...
                                                <h3 className="text-xl md:text-2xl font-black text-slate-800 mb-2 group-hover:text-emerald-600 transition-colors">{member.name}</h3>
                                                <p className="text-sky-600 font-bold text-sm bg-sky-50 px-5 py-1.5 rounded-full border border-sky-100/50">
                                                    {member.position_key}
                                                </p>
                                                <p className="text-slate-400 text-[10px] mt-6 font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">خبراء أبار جروب المعتمدون</p>
                                            </div>
                                        </div>
                                    ))}
                                </Slider>
